...
// local helper to respond JSON and exit
function jsonResp($code, $payload){
    http_response_code($code);
    echo json_encode($payload);
    exit;
}

// simple safety wrapper for input
function getJsonInput(){ $raw = file_get_contents('php://input'); $data = json_decode($raw, true); return is_array($data) ? $data : []; }

// require connector
$connectorPath = __DIR__ . '/../../Connectors/connector.php';
if (!is_readable($connectorPath)) {
    jsonResp(500, ['success'=>false,'error'=>'Server misconfiguration: connector missing']);
}
require_once $connectorPath;
try { $db = DB::get(); } catch (Throwable $e) {
    // cannot proceed, but log to local file
...
function require_auth($db){
    if (empty($_SESSION['user_id'])) {
        jsonResp(401, ['success'=>false,'error'=>'Not authenticated']);
    }
...
try {
    ...
    // Global fallback: log and return 500
...
function api_upload($db){
    ...
    // CSRF token must be present in POST (multipart/form-data)
    $csrf = $_POST['csrf_token'] ?? ($_SERVER['HTTP_X_CSRF_TOKEN'] ?? null);
    if (!validate_csrf($csrf)) {
        jsonResp(403, ['success'=>false,'error'=>'Invalid CSRF token']);
    }

    if (empty($_FILES['statement_pdf'])) {
        jsonResp(400, ['success'=>false,'error'=>'No file uploaded']);
    }
    $f = $_FILES['statement_pdf'];
    if (!is_uploaded_file($f['tmp_name'])) {
        jsonResp(400, ['success'=>false,'error'=>'Upload error']);
    }
    $max = 20 * 1024 * 1024;
    if ($f['size'] > $max) jsonResp(400, ['success'=>false,'error'=>'File too large']);
    ...
    if (!move_uploaded_file($f['tmp_name'], $stored)) {
        $db->writeLocalLog('error', 'move_failed', ['src'=>$f['tmp_name'],'dest'=>$stored]);
        jsonResp(500, ['success'=>false,'error'=>'Move failed']);
    }

    // compute sha and check duplicate
    $sha = hash_file('sha256', $stored);

    try {
        // check existing statement with same sha for the same user
        $existing = $db->fetch('SELECT id, parse_status FROM statements WHERE file_sha256 = ? AND user_id = ? LIMIT 1', [$sha, $userId]);
        if ($existing) {
            // Log upload attempt and return existing statement id (idempotent)
        ...
        $db->writeLocalLog('warning', 'duplicate_check_failed', ['err'=>$e->getMessage()]);
        // continue to insert to preserve provenance
    }
    ...
    try {
        $stmtId = $db->insertAndGetId(
            ...
            [ $userId, $f['name'], $stored, intval($f['size']), $sha ]
        );
    ...
    // optionally compress placeholder
    $compress = isset($_POST['compress']) && $_POST['compress'] === '1';
    if ($compress) {
        // no-op in demo, but log intention
        $db->logParse($stmtId, 'info', 'requested_compress', ['compress'=>true], $userId);
    }

    // For demo we parse synchronously (same as before). In production, queue a worker and return statement_id.
    try {
        parse_and_insert($db, $stmtId, $userId, $stored);
    } catch (Throwable $e) {
        // mark error and return id so UI can poll
...
function api_promote($db){
    ...
    $canonical = trim($data['canonical_name'] ?? '');
...
// ---------------- Parsing & grouping helpers (same logic as earlier, but wrapped with logging) ----------------
...
function process_buffer_line($line){
    ...
    // Try patterns: narration withdraw deposit balance OR narration withdraw deposit OR narration withdraw deposit balance
...
function parse_and_insert($db, $stmtId, $userId, $storedPath){
    ...
    try{
        ...
        foreach($txns as $t){
            $account_id = null; // optionally derive or assign from statement parsing
    ...
    // run grouping scanner (post-insert)
...
function run_grouping_scanner($db, $userId){
    // find unassigned transactions for the user
    ...
    foreach($candidates as $ck => $info){
        ...
        // check if alias already points to a counterparty
        ...
        if (!$cp_id) {
            // try existing counterparties by canonical name
        ...
        // assign transactions to this counterparty
        ...
        // update aggregation on counterparties
