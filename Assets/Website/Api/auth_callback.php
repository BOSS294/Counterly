...
if (isset($_GET['error'])) {
    ...
    try { DB::get()->logParse(null, 'error', 'google_oauth_error: ' . $err, ['query' => $_GET]); } catch(Throwable $e){}
...
if ($response === false) {
    try { DB::get()->logParse(null, 'error', 'oauth_token_request_failed', ['curl_err' => $curlErr]); } catch(Throwable $e){}
...
if (!$tokenData || empty($tokenData['access_token'])) {
    try { DB::get()->logParse(null, 'error', 'oauth_token_parsing_failed', ['response' => $response, 'http_code' => $httpCode]); } catch(Throwable $e){}
    http_response_code(500);
...
if ($uiResp === false) {
    try { DB::get()->logParse(null, 'error', 'userinfo_request_failed', ['curl_err' => $uErr]); } catch(Throwable $e){}
    http_response_code(500);
...
if (!$userinfo || empty($userinfo['sub'])) {
    try { DB::get()->logParse(null, 'error', 'userinfo_parsing_failed', ['response' => $uiResp]); } catch(Throwable $e){}
    http_response_code(500);
...
try {
    ...
    if ($existing) {
        ...
        if (!$userId) {
            ...
            $params = [':name'=>$name, ':email'=>$email, ':p'=>$provider, ':oid'=>$googleSub, ':avatar'=>$picture];
    ...
    header('Location: /app/dashboard.php', true, 302);
    exit;

} catch (Throwable $e) {
    try { DB::get()->logParse(null, 'error', 'auth_processing_failed', ['err' => $e->getMessage()]); } catch(Throwable $t){}
    http_response_code(500);
